...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://gitee.com/tanloong/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://gitee.com/tanloong/dotfiles/raw/master/progs.csv"
...
gitmakeinstall() {
	...
	sudo -u "$name" git clone "$1" "$dir" >/dev/null 2>&1 || { cd "$dir" || return 1 ; sudo -u "$name" git pull --force origin master;}
	cd "$dir" || exit 1
	make >/dev/null 2>&1
...
for x in curl ca-certificates base-devel git; do
	installpkg "$x"
done
...
git clone "$dotfilesrepo" "/home/$name/dotfiles" && "/home/#name/dotfiles/link.sh"
