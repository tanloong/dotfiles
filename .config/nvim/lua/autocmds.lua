...
local preview_stack_trace = function()
    ...
    local patterns = { 'File "([^"]+)", line (%d+)', '^([^:]+):(%d+)' }

    for _, pattern in ipairs(patterns) do
        filepath, lineno = string.match(line, pattern)
        if filepath and lineno then break end
    end

    if not filepath or not lineno then
    ...
    vim.cmd("e " .. cwd .. "/" .. filepath)
    local winnr = vim.api.nvim_get_current_win()
    vim.api.nvim_win_set_cursor(winnr, { tonumber(lineno), 1 })
    vim.cmd("wincmd j")
end
